...
export const toggleCategoryStatus = async (id: string): Promise<boolean> => {
  try {
    const response = await apiClient.patch(`/api/categories/${id}/status`);
    return response.data.success;
  } catch (error: any) {
...
export const setCategoryDiscount = async (id: string, discounts: any[]): Promise<boolean> => {
  try {
    const response = await apiClient.post(`/api/categories/${id}/discounts`, { discounts });
    return response.data.success;
  } catch (error: any) {
...
export const setAllCategoriesDiscount = async (discounts: any[]): Promise<boolean> => {
  try {
    const response = await apiClient.post('/api/categories/discounts/batch', { discounts });
    return response.data.success;
  } catch (error: any) {
