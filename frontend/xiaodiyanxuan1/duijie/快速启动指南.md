# 小迪严选 - 后端快速启动指南

## 快速概览

| 项目 | 说明 |
|-----|------|
| 接口总数 | 26个 |
| 认证方式 | JWT Bearer Token |
| 数据格式 | JSON |
| 必须HTTPS | 是（小程序要求） |

---

## 第一步：创建后端项目

```bash
mkdir xiaodi-backend && cd xiaodi-backend
npm init -y
npm install express mongoose jsonwebtoken axios cors dotenv
```

---

## 第二步：项目结构

```
xiaodi-backend/
├── server.js           # 入口文件
├── config/
│   └── db.js          # 数据库配置
├── models/            # 数据模型
│   ├── User.js
│   ├── Goods.js
│   ├── Order.js
│   ├── Cart.js
│   └── Address.js
├── routes/            # 路由
│   ├── auth.js
│   ├── goods.js
│   ├── cart.js
│   ├── orders.js
│   └── addresses.js
├── middleware/
│   └── auth.js        # JWT验证中间件
└── .env
```

---

## 第三步：核心代码示例

### server.js
```javascript
const express = require('express')
const mongoose = require('mongoose')
const cors = require('cors')
require('dotenv').config()

const app = express()
app.use(cors())
app.use(express.json())

// 数据库连接
mongoose.connect(process.env.MONGODB_URI)

// 路由
app.use('/api/auth', require('./routes/auth'))
app.use('/api/goods', require('./routes/goods'))
app.use('/api/cart', require('./routes/cart'))
app.use('/api/orders', require('./routes/orders'))
app.use('/api/addresses', require('./routes/addresses'))
app.use('/api/favorites', require('./routes/favorites'))
app.use('/api/bargain', require('./routes/bargain'))
app.use('/api/coupons', require('./routes/coupons'))

// 首页数据
app.get('/api/home', (req, res) => {
  res.json({ code: 0, data: { banners: [], hotGoods: [] } })
})

// 用户信息
app.get('/api/user/info', authMiddleware, (req, res) => {
  res.json({ code: 0, data: req.user })
})

const PORT = process.env.PORT || 5000
app.listen(PORT, () => console.log(`Server running on port ${PORT}`))
```

### JWT中间件 (middleware/auth.js)
```javascript
const jwt = require('jsonwebtoken')

module.exports = (req, res, next) => {
  const token = req.headers.authorization?.split(' ')[1]
  if (!token) {
    return res.status(401).json({ code: 401, message: '未登录' })
  }
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET)
    req.user = decoded
    next()
  } catch (err) {
    return res.status(401).json({ code: 401, message: 'token无效' })
  }
}
```

---

## 第四步：优先实现的接口

按重要程度排序，建议按此顺序实现：

### 🔴 必须实现（核心流程）
1. `POST /api/auth/wxlogin` - 微信登录
2. `GET /api/goods/list` - 商品列表
3. `GET /api/goods/:id` - 商品详情
4. `POST /api/orders` - 创建订单
5. `GET /api/orders` - 订单列表

### 🟡 重要功能
6. `GET /api/cart` - 购物车
7. `POST /api/cart` - 添加购物车
8. `PUT /api/cart/:id` - 更新购物车
9. `DELETE /api/cart/:id` - 删除购物车
10. `POST /api/orders/:id/cancel` - 取消订单

### 🟢 次要功能
11. 收藏相关 (3个)
12. 地址相关 (4个)
13. 砍价相关 (4个)
14. 优惠券 (1个)

---

## 第五步：响应格式统一

**成功响应：**
```json
{
  "code": 0,
  "data": { ... },
  "message": "success"
}
```

**错误响应：**
```json
{
  "code": 错误码,
  "message": "错误信息"
}
```

---

## 第六步：测试接口

使用 Postman 或 curl 测试：

```bash
# 测试商品列表
curl https://your-domain.com/api/goods/list

# 测试创建订单（需要token）
curl -X POST https://your-domain.com/api/orders \
  -H "Authorization: Bearer your_token" \
  -H "Content-Type: application/json" \
  -d '{"goods":[...],"totalPrice":9999}'
```

---

## 第七步：部署上线

1. 配置HTTPS证书
2. 设置环境变量
3. 启动服务
4. 小程序后台配置域名白名单
5. 修改前端 `config/api.js` 的 baseURL

---

## 联调注意事项

1. **所有接口必须返回JSON格式**
2. **认证接口需要验证JWT token**
3. **订单状态码：1待付款 2待发货 3待收货 4已完成 5已取消**
4. **图片URL必须是HTTPS**

---

## 有问题？

查看 `后端对接文档.md` 获取详细接口说明。
