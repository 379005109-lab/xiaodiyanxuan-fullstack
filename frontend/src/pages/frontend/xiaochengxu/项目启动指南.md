# 项目启动指南

## 📋 项目结构概览

```
xiaodiyanxuan/
├── 小程序前端/          # 微信小程序（已完成框架）
├── web-frontend/        # Web 前端（待开发）
├── backend/             # 后端服务（待开发）
└── 文档/
    ├── 后端项目开发提示词.md
    ├── Web前端项目开发提示词.md
    ├── 全栈对接指南.md
    ├── API接口速查表.md
    ├── 前端开发功能文档与API接口要求.md
    ├── API接口对接文档.md
    └── 项目启动指南.md（本文件）
```

---

## 🚀 快速开始

### 第一步：后端服务启动

#### 1. 创建后端项目

```bash
# 创建项目目录
mkdir backend
cd backend

# 初始化 Node.js 项目
npm init -y

# 安装依赖
npm install express mongoose jsonwebtoken bcryptjs dotenv cors helmet morgan axios multer joi
npm install -D nodemon
```

#### 2. 配置环境变量

创建 `.env` 文件：

```
PORT=5000
NODE_ENV=development
MONGODB_URI=mongodb://localhost:27017/xiaodiyanxuan
JWT_SECRET=your_jwt_secret_key_here_change_in_production
JWT_EXPIRE=7d
WECHAT_APPID=your_wechat_appid
WECHAT_APPSECRET=your_wechat_appsecret
CORS_ORIGIN=http://localhost:3000,http://localhost:8080,http://localhost:5173
```

#### 3. 创建基础项目结构

```bash
mkdir -p src/{config,models,routes,controllers,middleware,services,utils}
touch src/app.js server.js
```

#### 4. 启动开发服务器

```bash
# 添加 npm script 到 package.json
"scripts": {
  "dev": "nodemon server.js",
  "start": "node server.js"
}

# 启动
npm run dev
```

**预期输出**：
```
Server running on port 5000
MongoDB connected
```

---

### 第二步：Web 前端启动

#### 1. 创建 React 项目

```bash
# 使用 Vite 创建项目
npm create vite@latest web-frontend -- --template react-ts

cd web-frontend

# 安装依赖
npm install
npm install react-router-dom axios zustand react-hook-form zod @hookform/resolvers
npm install -D tailwindcss postcss autoprefixer
npm install lucide-react clsx class-variance-authority

# 初始化 Tailwind
npx tailwindcss init -p
```

#### 2. 配置环境变量

创建 `.env.local` 文件：

```
VITE_API_URL=http://localhost:5000
VITE_APP_NAME=小程序商城
VITE_APP_VERSION=1.0.0
```

#### 3. 启动开发服务器

```bash
npm run dev
```

**预期输出**：
```
  VITE v4.x.x  ready in xxx ms

  ➜  Local:   http://localhost:5173/
  ➜  press h to show help
```

---

### 第三步：小程序前端启动

#### 1. 配置 API 地址

编辑 `config/api.js`：

```javascript
const config = {
  baseURL: 'http://localhost:5000',  // 改为你的后端地址
  timeout: 10000,
  debug: true
}

module.exports = config
```

#### 2. 在微信开发者工具中打开

1. 打开微信开发者工具
2. 选择"打开"
3. 选择项目目录（xiaodiyanxuan1）
4. 点击"打开"

#### 3. 配置开发环境

在开发者工具中：
- 点击"详情" → "本地设置"
- 勾选"不校验合法域名、web-view（业务域名）、TLS 版本以及 HTTPS 证书"

---

## 🧪 测试各端连接

### 1. 后端 API 测试

使用 Postman 或 curl 测试：

```bash
# 测试后端是否运行
curl http://localhost:5000/api/home

# 预期响应
{
  "code": 0,
  "message": "success",
  "data": { ... }
}
```

### 2. Web 前端测试

1. 打开浏览器访问 `http://localhost:5173`
2. 打开浏览器开发者工具（F12）
3. 查看 Network 标签，确认 API 请求成功

### 3. 小程序测试

1. 在微信开发者工具中打开项目
2. 查看控制台输出
3. 确认 API 请求成功

---

## 📊 开发流程

### 阶段 1：后端开发（1-2 周）

**优先级**：🔴 最高

**任务**：
1. [ ] 数据库设计和建模
2. [ ] 认证系统（微信登录）
3. [ ] 商品模块 API
4. [ ] 订单模块 API
5. [ ] 其他模块 API
6. [ ] 测试和优化

**参考文档**：`后端项目开发提示词.md`

### 阶段 2：Web 前端开发（1-2 周）

**优先级**：🟡 中等

**任务**：
1. [ ] 项目初始化和配置
2. [ ] 基础框架搭建
3. [ ] 页面开发
4. [ ] 功能集成
5. [ ] 测试和优化

**参考文档**：`Web前端项目开发提示词.md`

### 阶段 3：小程序对接（1 周）

**优先级**：🟢 低

**任务**：
1. [ ] 配置后端 API 地址
2. [ ] 测试各个功能
3. [ ] 修复 bug
4. [ ] 性能优化

**参考文档**：`前端开发功能文档与API接口要求.md`

---

## 🔄 数据流示例

### 用户登录流程

```
小程序/Web → 调用 wx.login() / 获取 code
         ↓
      后端 → 验证 code，获取 openId
         ↓
      后端 → 生成 JWT Token
         ↓
小程序/Web → 保存 Token 到本地
         ↓
小程序/Web → 后续请求都带上 Token
```

### 商品购买流程

```
小程序/Web → 获取商品列表 GET /api/goods/list
         ↓
小程序/Web → 查看商品详情 GET /api/goods/:id
         ↓
小程序/Web → 添加到购物车 POST /api/cart
         ↓
小程序/Web → 查看购物车 GET /api/cart
         ↓
小程序/Web → 创建订单 POST /api/orders
         ↓
小程序/Web → 查看订单 GET /api/orders/:orderId
```

---

## 🛠️ 常用命令

### 后端

```bash
# 启动开发服务器
npm run dev

# 启动生产服务器
npm start

# 运行测试
npm test

# 构建
npm run build
```

### Web 前端

```bash
# 启动开发服务器
npm run dev

# 构建生产版本
npm run build

# 预览生产版本
npm run preview

# 运行测试
npm test

# 代码检查
npm run lint
```

### 小程序

在微信开发者工具中：
- 编译：Ctrl + B（Windows）/ Cmd + B（Mac）
- 清除缓存：工具 → 清除缓存 → 清除文件缓存
- 上传：上传 → 输入版本号 → 上传

---

## 📝 数据库初始化

### 1. 安装 MongoDB

**Windows**：
```bash
# 使用 Chocolatey
choco install mongodb-community

# 或下载安装程序
# https://www.mongodb.com/try/download/community
```

**Mac**：
```bash
# 使用 Homebrew
brew tap mongodb/brew
brew install mongodb-community

# 启动 MongoDB
brew services start mongodb-community
```

**Linux**：
```bash
# Ubuntu/Debian
sudo apt-get install -y mongodb

# 启动 MongoDB
sudo systemctl start mongodb
```

### 2. 创建数据库

```bash
# 连接到 MongoDB
mongosh

# 创建数据库
use xiaodiyanxuan

# 创建集合
db.createCollection("users")
db.createCollection("goods")
db.createCollection("orders")
# ... 其他集合
```

### 3. 导入示例数据

在后端项目中创建 `scripts/seed.js`：

```javascript
const mongoose = require('mongoose')
const Goods = require('../src/models/Goods')

async function seedData() {
  await mongoose.connect(process.env.MONGODB_URI)
  
  // 清空现有数据
  await Goods.deleteMany({})
  
  // 插入示例数据
  await Goods.insertMany([
    {
      id: 'g1',
      name: '莫兰迪沙发',
      code: 'FS-SF3201',
      price: 2699,
      // ... 其他字段
    },
    // ... 更多商品
  ])
  
  console.log('数据导入成功')
  process.exit(0)
}

seedData().catch(err => {
  console.error(err)
  process.exit(1)
})
```

运行：
```bash
node scripts/seed.js
```

---

## 🔐 安全配置

### 1. 环境变量

**不要在代码中硬编码敏感信息**：

```javascript
// ❌ 错误
const JWT_SECRET = 'my_secret_key'

// ✅ 正确
const JWT_SECRET = process.env.JWT_SECRET
```

### 2. CORS 配置

```javascript
// 后端 app.js
const cors = require('cors')

app.use(cors({
  origin: process.env.CORS_ORIGIN.split(','),
  credentials: true
}))
```

### 3. HTTPS

生产环境必须使用 HTTPS：

```javascript
// 使用 Let's Encrypt 获取免费证书
// https://letsencrypt.org/

// 或使用 Nginx 反向代理
```

---

## 📊 监控和日志

### 1. 后端日志

```javascript
// 使用 morgan 记录请求日志
const morgan = require('morgan')
app.use(morgan('combined'))

// 使用 winston 记录应用日志
const winston = require('winston')
const logger = winston.createLogger({
  level: 'info',
  format: winston.format.json(),
  transports: [
    new winston.transports.File({ filename: 'error.log', level: 'error' }),
    new winston.transports.File({ filename: 'combined.log' })
  ]
})
```

### 2. 前端错误追踪

```javascript
// 使用 Sentry 追踪错误
import * as Sentry from "@sentry/react"

Sentry.init({
  dsn: "your-sentry-dsn",
  environment: process.env.NODE_ENV
})
```

---

## 🚀 部署清单

### 后端部署

- [ ] 配置生产环境变量
- [ ] 配置 HTTPS 证书
- [ ] 配置数据库备份
- [ ] 配置日志收集
- [ ] 配置监控告警
- [ ] 配置 CI/CD 流程
- [ ] 性能测试
- [ ] 安全审计

### Web 前端部署

- [ ] 构建生产版本
- [ ] 配置 CDN
- [ ] 配置缓存策略
- [ ] 配置 Gzip 压缩
- [ ] 性能测试
- [ ] SEO 优化

### 小程序部署

- [ ] 配置合法域名
- [ ] 上传代码
- [ ] 提交审核
- [ ] 发布上线

---

## 📞 故障排查

### 后端无法启动

```bash
# 检查端口是否被占用
lsof -i :5000  # Mac/Linux
netstat -ano | findstr :5000  # Windows

# 杀死占用端口的进程
kill -9 <PID>  # Mac/Linux
taskkill /PID <PID> /F  # Windows
```

### 数据库连接失败

```bash
# 检查 MongoDB 是否运行
mongosh

# 检查连接字符串
# mongodb://localhost:27017/xiaodiyanxuan
```

### 前端 API 请求失败

1. 检查后端是否运行
2. 检查 API 地址是否正确
3. 检查 CORS 配置
4. 查看浏览器控制台错误信息

### 小程序请求失败

1. 检查域名配置
2. 检查 token 是否正确
3. 查看开发者工具控制台错误信息

---

## 📚 相关资源

- **后端开发提示词**：`后端项目开发提示词.md`
- **Web 前端开发提示词**：`Web前端项目开发提示词.md`
- **全栈对接指南**：`全栈对接指南.md`
- **API 接口速查表**：`API接口速查表.md`

---

## ✅ 启动检查清单

- [ ] 后端服务运行正常（http://localhost:5000）
- [ ] MongoDB 数据库连接成功
- [ ] Web 前端开发服务运行正常（http://localhost:5173）
- [ ] 小程序在开发者工具中打开
- [ ] 三端都能访问 API
- [ ] 登录功能正常
- [ ] 商品列表能正常加载
- [ ] 购物车功能正常
- [ ] 订单创建功能正常

---

**最后更新**：2024年11月17日  
**版本**：1.0.0

如有问题，请参考相关文档或联系开发团队。
