# 后端项目开发提示词 - Node.js + MongoDB 架构

## 📋 项目概述

**项目名称**：小程序商城后端服务  
**技术栈**：Node.js + Express + MongoDB  
**主要功能**：提供小程序和 Web 前端的 API 接口服务  
**开发周期**：完整的后端系统搭建

---

## 🎯 核心需求

### 1. 项目初始化

**框架选择**：Express.js  
**数据库**：MongoDB（使用 Mongoose ODM）  
**认证**：JWT Token（微信登录集成）  
**部署**：支持 Docker 容器化部署

**必需的依赖包**：
```json
{
  "express": "^4.18.2",
  "mongoose": "^7.0.0",
  "jsonwebtoken": "^9.0.0",
  "bcryptjs": "^2.4.3",
  "dotenv": "^16.0.3",
  "cors": "^2.8.5",
  "helmet": "^7.0.0",
  "morgan": "^1.10.0",
  "axios": "^1.4.0",
  "multer": "^1.4.5-lts.1",
  "joi": "^17.9.2"
}
```

### 2. 项目结构

```
backend/
├── src/
│   ├── config/              # 配置文件
│   │   ├── database.js      # MongoDB 连接配置
│   │   ├── jwt.js           # JWT 配置
│   │   └── wechat.js        # 微信 API 配置
│   ├── models/              # 数据库模型
│   │   ├── User.js          # 用户模型
│   │   ├── Goods.js         # 商品模型
│   │   ├── Order.js         # 订单模型
│   │   ├── Cart.js          # 购物车模型
│   │   ├── Favorite.js      # 收藏模型
│   │   ├── Address.js       # 地址模型
│   │   ├── Coupon.js        # 优惠券模型
│   │   ├── Bargain.js       # 砍价模型
│   │   ├── Category.js      # 分类模型
│   │   ├── Style.js         # 风格模型
│   │   └── Package.js       # 套餐模型
│   ├── routes/              # 路由文件
│   │   ├── auth.js          # 认证路由
│   │   ├── user.js          # 用户路由
│   │   ├── goods.js         # 商品路由
│   │   ├── orders.js        # 订单路由
│   │   ├── cart.js          # 购物车路由
│   │   ├── favorites.js     # 收藏路由
│   │   ├── addresses.js     # 地址路由
│   │   ├── coupons.js       # 优惠券路由
│   │   ├── bargain.js       # 砍价路由
│   │   ├── home.js          # 首页路由
│   │   └── packages.js      # 套餐路由
│   ├── controllers/         # 控制器
│   │   ├── authController.js
│   │   ├── userController.js
│   │   ├── goodsController.js
│   │   ├── orderController.js
│   │   ├── cartController.js
│   │   ├── favoriteController.js
│   │   ├── addressController.js
│   │   ├── couponController.js
│   │   ├── bargainController.js
│   │   ├── homeController.js
│   │   └── packageController.js
│   ├── middleware/          # 中间件
│   │   ├── auth.js          # 认证中间件
│   │   ├── errorHandler.js  # 错误处理中间件
│   │   └── validation.js    # 数据验证中间件
│   ├── services/            # 业务逻辑服务
│   │   ├── wechatService.js # 微信服务
│   │   ├── orderService.js  # 订单服务
│   │   └── bargainService.js# 砍价服务
│   ├── utils/               # 工具函数
│   │   ├── response.js      # 统一响应格式
│   │   ├── logger.js        # 日志工具
│   │   └── validators.js    # 验证工具
│   └── app.js               # 应用主文件
├── .env                     # 环境变量
├── .env.example             # 环境变量示例
├── package.json
├── server.js                # 启动文件
└── README.md
```

---

## 🔌 API 接口规范

### 1. 统一响应格式

所有接口必须返回统一格式：

```javascript
// 成功响应
{
  "code": 0,
  "message": "success",
  "data": { /* 实际数据 */ }
}

// 失败响应
{
  "code": 400,
  "message": "错误信息描述",
  "data": null
}
```

### 2. HTTP 状态码规范

- `200`：请求成功
- `400`：请求参数错误
- `401`：未授权（需要登录）
- `403`：禁止访问
- `404`：资源不存在
- `500`：服务器错误

### 3. 认证方式

需要认证的接口，请求头需包含：
```
Authorization: Bearer {token}
```

Token 由微信登录接口返回，前端存储在本地。

---

## 📦 数据模型设计

### 1. 用户模型（User）

```javascript
{
  _id: ObjectId,
  openId: String,           // 微信 openId（唯一）
  unionId: String,          // 微信 unionId
  nickname: String,         // 昵称
  avatar: String,           // 头像
  phone: String,            // 手机号
  email: String,            // 邮箱
  token: String,            // JWT Token
  createdAt: Date,
  updatedAt: Date
}
```

### 2. 商品模型（Goods）

```javascript
{
  _id: ObjectId,
  id: String,               // 商品ID（自定义）
  name: String,             // 商品名称
  code: String,             // 商品编码
  price: Number,            // 基础价格
  thumb: String,            // 缩略图
  images: [String],         // 商品图片数组
  detailImages: [String],   // 详情图片数组
  category: String,         // 分类名称
  categoryId: String,       // 分类ID
  style: String,            // 风格
  styleId: String,          // 风格ID
  description: String,      // 商品描述
  stock: Number,            // 库存
  sales: Number,            // 销量
  
  // 规格配置
  sizes: [{
    id: String,
    name: String,
    dims: String,           // 尺寸描述
    img: String,
    extra: Number           // 额外加价
  }],
  
  materialsGroups: [{
    id: String,
    name: String,
    extra: Number,
    better: Boolean,
    img: String,
    colors: [{
      id: String,
      name: String
    }]
  }],
  
  fills: [{
    id: String,
    name: String,
    extra: Number,
    better: Boolean,
    img: String
  }],
  
  frames: [{
    id: String,
    name: String,
    extra: Number,
    better: Boolean,
    img: String
  }],
  
  legs: [{
    id: String,
    name: String,
    extra: Number,
    better: Boolean,
    img: String
  }],
  
  createdAt: Date,
  updatedAt: Date
}
```

### 3. 订单模型（Order）

```javascript
{
  _id: ObjectId,
  id: String,               // 订单ID
  orderNo: String,          // 订单号（唯一）
  userId: ObjectId,         // 用户ID
  type: String,             // 订单类型：normal 或 package
  status: Number,           // 状态：1-待付款, 2-待发货, 3-待收货, 4-已完成, 5-已取消
  totalPrice: Number,       // 订单总价
  
  goods: [{
    id: String,
    name: String,
    code: String,
    price: Number,
    count: Number,
    thumb: String,
    specs: {
      size: String,
      material: String,
      materialColor: String,
      fill: String,
      frame: String,
      leg: String
    }
  }],
  
  // 收货信息
  name: String,             // 收货人姓名
  phone: String,            // 收货人电话
  address: String,          // 收货地址
  
  // 时间戳
  createTime: Date,
  payTime: Date,
  shipTime: Date,
  completeTime: Date,
  
  createdAt: Date,
  updatedAt: Date
}
```

### 4. 购物车模型（Cart）

```javascript
{
  _id: ObjectId,
  userId: ObjectId,
  goodsId: String,
  goodsName: String,
  price: Number,
  count: Number,
  specs: {
    size: String,
    material: String,
    materialColor: String,
    fill: String,
    frame: String,
    leg: String
  },
  createdAt: Date,
  updatedAt: Date
}
```

### 5. 收藏模型（Favorite）

```javascript
{
  _id: ObjectId,
  userId: ObjectId,
  goodsId: String,
  goodsName: String,
  thumb: String,
  price: Number,
  createdAt: Date
}
```

### 6. 地址模型（Address）

```javascript
{
  _id: ObjectId,
  userId: ObjectId,
  name: String,
  phone: String,
  province: String,
  city: String,
  district: String,
  address: String,
  isDefault: Boolean,
  createdAt: Date,
  updatedAt: Date
}
```

### 7. 优惠券模型（Coupon）

```javascript
{
  _id: ObjectId,
  code: String,             // 优惠券码
  type: String,             // 类型：fixed（固定金额）或 percent（百分比）
  value: Number,            // 优惠值
  minAmount: Number,        // 最低消费金额
  maxAmount: Number,        // 最高优惠金额
  validFrom: Date,          // 有效期开始
  validTo: Date,            // 有效期结束
  usageLimit: Number,       // 使用次数限制
  usageCount: Number,       // 已使用次数
  createdAt: Date,
  updatedAt: Date
}
```

### 8. 砍价模型（Bargain）

```javascript
{
  _id: ObjectId,
  goodsId: String,
  goodsName: String,
  originalPrice: Number,    // 原价
  targetPrice: Number,      // 目标价格
  currentPrice: Number,     // 当前价格
  userId: ObjectId,         // 发起人
  status: String,           // 状态：active 或 completed
  helpers: [{
    userId: ObjectId,
    bargainAmount: Number,  // 砍价金额
    createdAt: Date
  }],
  createdAt: Date,
  updatedAt: Date
}
```

---

## 🔌 完整 API 接口列表

### 1. 认证接口

#### 1.1 微信登录
```
POST /api/auth/wxlogin
请求体：{ code: String }
响应：{ token: String, userInfo: Object }
认证：否
```

#### 1.2 获取用户信息
```
GET /api/user/info
响应：{ id, openId, nickname, avatar, phone, email }
认证：是
```

---

### 2. 首页接口

#### 2.1 获取首页数据
```
GET /api/home
响应：{
  fullscreenImage: String,
  newProductsImage: String,
  banners: [String],
  styles: [Object],
  hots: [Object]
}
认证：否
```

---

### 3. 商品接口

#### 3.1 获取商品列表
```
GET /api/goods/list
查询参数：
  - page: Number (默认 1)
  - pageSize: Number (默认 10)
  - category: String (可选)
  - style: String (可选)
  - sort: String (可选：price_asc, price_desc, sales_desc)

响应：{
  list: [Object],
  total: Number,
  page: Number,
  pageSize: Number
}
认证：否
```

#### 3.2 搜索商品
```
GET /api/goods/search
查询参数：
  - keyword: String
  - page: Number (默认 1)
  - pageSize: Number (默认 10)

响应：{
  list: [Object],
  total: Number
}
认证：否
```

#### 3.3 获取商品详情
```
GET /api/goods/:id
响应：完整的商品对象（包含所有规格配置）
认证：否
```

#### 3.4 获取分类列表
```
GET /api/categories
响应：[{ id, name, icon, sort }]
认证：否
```

#### 3.5 获取风格列表
```
GET /api/styles
响应：[{ id, name, icon, sort }]
认证：否
```

---

### 4. 购物车接口

#### 4.1 获取购物车
```
GET /api/cart
响应：[{ _id, goodsId, goodsName, price, count, specs }]
认证：是
```

#### 4.2 添加到购物车
```
POST /api/cart
请求体：{
  goodsId: String,
  count: Number,
  specs: {
    size: String,
    material: String,
    materialColor: String,
    fill: String,
    frame: String,
    leg: String
  }
}
响应：{ _id, message: "添加成功" }
认证：是
```

#### 4.3 更新购物车商品数量
```
PUT /api/cart/:cartId
请求体：{ count: Number }
响应：{ message: "更新成功" }
认证：是
```

#### 4.4 删除购物车商品
```
DELETE /api/cart/:cartId
响应：{ message: "删除成功" }
认证：是
```

---

### 5. 收藏接口

#### 5.1 获取收藏列表
```
GET /api/favorites
查询参数：
  - page: Number (默认 1)
  - pageSize: Number (默认 10)

响应：{
  list: [Object],
  total: Number
}
认证：是
```

#### 5.2 添加收藏
```
POST /api/favorites
请求体：{ goodsId: String }
响应：{ message: "收藏成功" }
认证：是
```

#### 5.3 取消收藏
```
DELETE /api/favorites/:goodsId
响应：{ message: "取消收藏成功" }
认证：是
```

---

### 6. 订单接口

#### 6.1 创建订单
```
POST /api/orders
请求体：{
  type: String,           // "normal" 或 "package"
  totalPrice: Number,
  goods: [{
    id: String,
    name: String,
    price: Number,
    count: Number,
    specs: Object
  }],
  name: String,           // 收货人
  phone: String,
  address: String
}
响应：{ orderId: String, orderNo: String }
认证：是
```

#### 6.2 获取订单列表
```
GET /api/orders
查询参数：
  - status: Number (可选：1-5)
  - page: Number (默认 1)
  - pageSize: Number (默认 10)

响应：{
  list: [Object],
  total: Number
}
认证：是
```

#### 6.3 获取订单详情
```
GET /api/orders/:orderId
响应：完整的订单对象
认证：是
```

#### 6.4 取消订单
```
POST /api/orders/:orderId/cancel
响应：{ message: "取消成功" }
认证：是
```

#### 6.5 确认收货
```
POST /api/orders/:orderId/confirm
响应：{ message: "确认成功" }
认证：是
```

---

### 7. 地址接口

#### 7.1 获取地址列表
```
GET /api/addresses
响应：[{ _id, name, phone, province, city, district, address, isDefault }]
认证：是
```

#### 7.2 添加地址
```
POST /api/addresses
请求体：{
  name: String,
  phone: String,
  province: String,
  city: String,
  district: String,
  address: String,
  isDefault: Boolean
}
响应：{ _id, message: "添加成功" }
认证：是
```

#### 7.3 更新地址
```
PUT /api/addresses/:addressId
请求体：{ 同添加地址 }
响应：{ message: "更新成功" }
认证：是
```

#### 7.4 删除地址
```
DELETE /api/addresses/:addressId
响应：{ message: "删除成功" }
认证：是
```

---

### 8. 优惠券接口

#### 8.1 获取优惠券列表
```
GET /api/coupons
查询参数：
  - status: String (可选：available, used, expired)

响应：[{
  _id, code, type, value, minAmount, maxAmount,
  validFrom, validTo, usageLimit, usageCount
}]
认证：是
```

---

### 9. 砍价接口

#### 9.1 获取砍价商品列表
```
GET /api/bargain/goods
查询参数：
  - page: Number (默认 1)
  - pageSize: Number (默认 10)

响应：{
  list: [Object],
  total: Number
}
认证：否
```

#### 9.2 发起砍价
```
POST /api/bargain/start
请求体：{ goodsId: String }
响应：{ bargainId: String }
认证：是
```

#### 9.3 获取我的砍价列表
```
GET /api/bargain/my
响应：[Object]
认证：是
```

#### 9.4 帮好友砍价
```
POST /api/bargain/:bargainId/help
响应：{ message: "砍价成功", currentPrice: Number }
认证：是
```

---

### 10. 套餐接口

#### 10.1 获取套餐列表
```
GET /api/packages
查询参数：
  - page: Number (默认 1)
  - pageSize: Number (默认 10)

响应：{
  list: [Object],
  total: Number
}
认证：否
```

#### 10.2 获取套餐详情
```
GET /api/packages/:packageId
响应：完整的套餐对象
认证：否
```

---

## 🛠️ 开发要求

### 1. 中间件配置

- **CORS**：允许小程序和 Web 前端跨域请求
- **认证中间件**：验证 JWT Token
- **错误处理**：统一的错误处理和日志记录
- **请求验证**：使用 Joi 进行数据验证

### 2. 微信集成

- 使用微信官方 API 验证登录 code
- 获取用户的 openId 和 unionId
- 生成 JWT Token 返回给前端

### 3. 数据验证

- 所有输入数据必须进行验证
- 返回清晰的错误信息

### 4. 错误处理

- 统一的错误响应格式
- 详细的错误日志记录

### 5. 性能优化

- 使用数据库索引
- 实现分页查询
- 缓存常用数据（可选）

### 6. 安全性

- 密码加密存储（bcryptjs）
- JWT Token 验证
- 输入数据验证和清理
- 防止 SQL 注入（MongoDB 注入）

---

## 📝 环境变量配置

创建 `.env` 文件：

```
# 服务器配置
PORT=5000
NODE_ENV=development

# 数据库配置
MONGODB_URI=mongodb://localhost:27017/xiaodiyanxuan
MONGODB_USER=
MONGODB_PASSWORD=

# JWT 配置
JWT_SECRET=your_jwt_secret_key_here
JWT_EXPIRE=7d

# 微信配置
WECHAT_APPID=your_wechat_appid
WECHAT_APPSECRET=your_wechat_appsecret

# 日志配置
LOG_LEVEL=debug

# CORS 配置
CORS_ORIGIN=http://localhost:3000,http://localhost:8080
```

---

## 🚀 启动和部署

### 1. 本地开发

```bash
npm install
npm run dev
```

### 2. 生产环境

```bash
npm run build
npm start
```

### 3. Docker 部署

创建 `Dockerfile`：

```dockerfile
FROM node:18-alpine

WORKDIR /app

COPY package*.json ./
RUN npm ci --only=production

COPY src ./src

EXPOSE 5000

CMD ["node", "server.js"]
```

---

## ✅ 开发检查清单

- [ ] 项目初始化和依赖安装
- [ ] MongoDB 数据库连接
- [ ] 用户认证系统（微信登录）
- [ ] JWT Token 生成和验证
- [ ] 商品模块（列表、详情、搜索）
- [ ] 购物车模块
- [ ] 订单模块
- [ ] 收藏模块
- [ ] 地址管理模块
- [ ] 优惠券模块
- [ ] 砍价模块
- [ ] 套餐模块
- [ ] 首页数据接口
- [ ] 错误处理和日志
- [ ] 数据验证
- [ ] API 文档生成（Swagger）
- [ ] 单元测试
- [ ] 集成测试
- [ ] 性能测试
- [ ] 安全审计

---

## 📚 参考资源

- Express.js 官方文档：https://expressjs.com/
- Mongoose 官方文档：https://mongoosejs.com/
- JWT 认证：https://jwt.io/
- 微信小程序开发文档：https://developers.weixin.qq.com/miniprogram/

---

## ⚠️ 重要提示

1. **HTTPS 要求**：生产环境必须使用 HTTPS
2. **域名备案**：小程序使用的域名必须已备案
3. **数据安全**：敏感信息不要硬编码在代码中
4. **API 限流**：建议实现 API 限流防止滥用
5. **日志记录**：记录所有重要操作便于调试和审计

---

如有问题，请参考相关文档或联系开发团队。
