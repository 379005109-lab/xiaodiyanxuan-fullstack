    ...
    try {
      ...
      for (const [productName, productData] of productMap.entries()) {
        ...
        if (existingProduct) {
          ...
          const newProduct = { name: productData.name, description: `${productData.name}系列商品`, category: productData.category as any, style: 'modern' as any, basePrice: productData.skus[0].price || 0, images: [], skus: productData.skus.map((sku: any, index: number) => ({ _id: `sku-${Date.now()}-${index}`, code: sku.code || `SKU-${Date.now()}-${index}`, color: sku.spec || '默认', spec: sku.spec, length: sku.length, width: sku.width, height: sku.height, material: typeof sku.material === 'string' ? { fabric: sku.material || '', filling: '', frame: '', leg: '' } : (sku.material || { fabric: '', filling: '', frame: '', leg: '' }), stock: sku.stock, price: sku.price, images: [], isPro: sku.isPro, proFeature: sku.proFeature, discountPrice: sku.discountPrice })), isCombo: false, tags: [], specifications, status: 'active' as any, views: 0, sales: 0, rating: 0, reviews: 0 };
